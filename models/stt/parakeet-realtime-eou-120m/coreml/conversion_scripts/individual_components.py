...
class EncoderStreamingWrapper(torch.nn.Module):
    ...
    def forward(
        ...
        Args:
            features: Mel spectrogram [B, D, T]
            length: Sequence lengths [B]
            cache_last_channel: Channel cache [B, num_layers, cache_size, d_model]
            cache_last_time: Time cache [B, num_layers, d_model, conv_context]
            cache_last_channel_len: Cache fill levels [B] (int32)

        Returns:
            encoded: Encoder output [B, D, T_enc]
            encoded_lengths: Output lengths [B] (int32)
            cache_last_channel_next: Updated channel cache
            cache_last_time_next: Updated time cache
            cache_last_channel_len_next: Updated cache fill levels (int32)
        """
        ...
        # NeMo expects int64 for cache_len, convert from int32
        ...
        if len(result) == 2:
            # No cache returned (shouldn't happen with cache input)
            encoded, encoded_lengths = result
            return (
                encoded,
        ...
        cache_last_channel_next = cache_last_channel_next.transpose(0, 1)
        cache_last_time_next = cache_last_time_next.transpose(0, 1)

        # Convert int64 outputs back to int32 for CoreML compatibility
        return (
            encoded,
            encoded_lengths.to(dtype=torch.int32),
            cache_last_channel_next,
            cache_last_time_next,
            cache_last_channel_len_next.to(dtype=torch.int32),
        )
